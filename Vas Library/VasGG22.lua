...
local function isHoldingAim()
    ...
    return isKeyDown(VasGG.Options.AimKey)
end
...
function VasGG.AimbotStep()
    ...
    if opt.AimShowFOV and opt.AimEnabled then
        if not fovCircle then
            fovCircle = newDrawing("Circle", {Visible = false, Thickness = 1, Color = Color3.fromRGB(255,255,255), Filled = false, NumSides = 48})
        end
        local mouseLoc = UserInputService:GetMouseLocation()
    ...
    if not opt.AimEnabled or not isHoldingAim() then return end
...
local function isHoldingTrigger()
    ...
    return isKeyDown(opt.TriggerbotKey)
end
...
function VasGG.TriggerbotStep()
    ...
    local now = tick()
    if now - lastTriggerTime < opt.TriggerbotDelay then return end
...
function VasGG.InitTriggerbot()
    if triggerbotConnection then triggerbotConnection:Disconnect() end
    triggerbotConnection = RunService.RenderStepped:Connect(VasGG.TriggerbotStep)
end
